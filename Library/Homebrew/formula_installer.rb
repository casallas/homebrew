...
class FormulaInstaller
  ...
  def self.external_dep_check dep, type
    case type
      when :python then %W{#{SystemCommand.env} -c import\ #{dep}}
      when :jruby then %W{#{SystemCommand.env} jruby -rubygems -e require\ '#{dep}'}
      when :ruby then %W{#{SystemCommand.env} ruby -rubygems -e require\ '#{dep}'}
      when :perl then %W{#{SystemCommand.env} perl -e use\ #{dep}}
    end
  end
